...
export const createRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const role = await findOrCreateRole(name, guard, {
      roleableId,
      roleableType,
    });
    return res.json(success("role created", bigintToString(role)));
  }
);

export const getRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
...
export const createPermissionHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { name, guard = "web" } = req.body;
    const perm = await findOrCreatePermission(name, guard);

    // Convert BigInt values to string before sending JSON
    const serializedPerm = JSON.parse(
      JSON.stringify(perm, (_, value) =>
        typeof value === "bigint" ? value.toString() : value
      )
    );

    return res.json(success("permission created", serializedPerm));
  }
);

export const getPermissionHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { name } = req.params;
...
export const givePermissionToRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { role } = req.params;
    const { permission, guard = "web", roleableId, roleableType } = req.body;
    await givePermissionToRole(role, permission, guard, {
      roleableId,
      roleableType,
    });
...
export const revokePermissionFromRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { role, permission } = req.params;
    const { guard = "web", roleableId, roleableType } = req.body;
    await revokePermissionFromRole(role, permission, guard, {
      roleableId,
      roleableType,
    });
...
export const assignRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { userId } = req.params;
    const { role, guard = "web", roleableId, roleableType } = req.body;
    await assignRole(DEFAULT_MODEL_TYPE, BigInt(userId), role, guard, {
      roleableId,
      roleableType,
    });
...
export const removeRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const { guard = "web", roleableId, roleableType } = req.body;
    await removeRole(DEFAULT_MODEL_TYPE, BigInt(userId), role, guard, {
      roleableId,
      roleableType,
    });
...
export const hasRoleHandler = asyncHandler(
  async (req: Request, res: Response) => {
    const { userId, role } = req.params;
    const { guard = "web", roleableId, roleableType } = req.query as any;
...
export const hasPermissionHandler = asyncHandler(
  async (req: Request, res: Response) => {
